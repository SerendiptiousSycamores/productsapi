...
var query = `SELECT * FROM skus INNER JOIN photos ON skus.style_id=photos.style_id WHERE skus.style_id IN (SELECT id FROM style where product_id=100 )`;
...
/* INDEXING */
// CREATE INDEX product_id_index_f ON features (product_id, feature, value);
// CREATE INDEX product_id_index_s ON style (product_id);
// CREATE INDEX style_id_index_st ON style (style_id);
// CREATE INDEX style_id_index_p ON photos (style_id);
// CREATE INDEX style_id_index_s ON skus (style_id, size, quantity);
